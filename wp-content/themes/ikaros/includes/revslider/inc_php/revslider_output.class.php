<?php
	...
	class RevSliderOutput{
		...
		public function putJS(){
			...
			$rev_js = '<script type="text/javascript">
				...
				tpj("#' . $this->sliderHtmlID . '").show().revolution(
					{
						delay:' .  $this->slider->getParam("delay","9000") . ',
						startwidth:' .  $this->slider->getParam("width","900") . ',
						startheight:' .  $this->slider->getParam("height","300") . ',
						...
						navigationType:"' .  $this->slider->getParam("navigaion_type","none") . '",
						navigationArrows:"' .  $this->slider->getParam("navigation_arrows","nexttobullets") . '",
						navigationStyle:"' .  $this->slider->getParam("navigation_style","round") . '",
						
						touchenabled:"' .  $this->slider->getParam("touchenabled","on") . '",
						onHoverStop:"' .  $this->slider->getParam("stop_on_hover","on") . '",
						
						navOffsetHorizontal:' .  $this->slider->getParam("nav_offset_hor","0") . ',
						navOffsetVertical:' .  $this->slider->getParam("nav_offset_vert","20") . ',
						
						shadow:' .  $this->slider->getParam("shadow_type","2") . ',
						fullWidth:"' .  $this->slider->getParam("fullwidth","on") . '"
					});
		...
		private function putResponsitiveStyles(){
			...
			?>
			   ...
			   @media only screen and (min-width: <?php echo $item["minWidth"]?>px) <?php echo $strMaxWidth?> {
			 		  ...
			 		  .bannercontainer {width:<?php echo $item["sliderWidth"]?>px;}
			   }
			
			<?php 
			endforeach;

			echo '.ie8 #' . $this->sliderHtmlID . ', .ie8 #' . $this->sliderHtmlID_wrapper . '{width:' . $arrItems[0]['sliderWidth'] . 'px;height:' . $arrItems[0]['sliderHeight'] . 'px;} .ie8 .bannercontainer {width:' . $arrItems[0]['sliderWidth'] . 'px;}';
