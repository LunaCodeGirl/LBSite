(function($){
	$(function() {
		...
		function insertButton(){
			var data = '[rb_button style="' + $('#rbButtonsStyle').val() + '" link="' + $('#rbButtonsLink').val() + '" target="' + $('#rbButtonsTarget').val() + '" label="' + $('#rbButtonsLabel').val()+ '"] [/rb_button]';
			return data;
		}
		...
		function insertDropcap(){
			var data = '[rb_dropcap]' + $('#rbDropcapLetter').val() + '[/rb_dropcap]';
			return data;
		}
		...
		function insertLatePosts(){
			var data= '[rb_latest_posts no="' + $('#rbLatePostsNo').val() + '" /]';
			return data;
		}

		function insertLateProjects(){
			var data= '[rb_latest_projects no="' + $('#rbLateProjectsNo').val() + '" /]';
			return data;
		}
		...
		function insertMark(html){
			var data;
			if(html)
				data = '[rb_highlight style="' + $('#rbMarkStyle').val() + '"]' + html + '[/rb_highlight]';
			else
				data = '[rb_highlight style="' + $('#rbMarkStyle').val() + '"]Your content here[/rb_highlight]';
			return data;
		}
		...
		function insertPricingTable(){
			var data = '[rb_pricing_table fields="'  + $('#rbPricingTableFields').val() + '"]'
			$('#rbPricingTable').children('div.boxes').children('div').each(function(){
				if(!$(this).hasClass('nochild')){
					data += '[rb_table_column style="' + $(this).find('.rbPricingTableStyle').val() + '"]';
					data += '[rb_table_price title="' + $(this).find('.rbPricingTableTitle').val() + '" subtitle="' + $(this).find('.rbPricingTablePriceLine').val() + '" sign="' + $(this).find('.rbPricingTablePriceSign').val() + '"]' + $(this).find('.rbPricingTablePrice').val() + '[/rb_table_price]';
		...
		function insertSocial(){
			...
			if($('#rbSocialRSS').val() != '')
				data += '[rb_link type="rss" href="' + $('#rbSocialRSS').val() + '" /]';
			if($('#rbSocialFacebook').val() != '')
				data += '[rb_link type="facebook" href="' + $('#rbSocialFacebook').val() + '" /]';
			if($('#rbSocialTwitter').val() != '')
				data += '[rb_link type="twitter" href="' + $('#rbSocialTwitter').val() + '" /]';
			if($('#rbSocialDribbble').val() != '')
				data += '[rb_link type="dribbble" href="' + $('#rbSocialDribbble').val() + '" /]';
			if($('#rbSocialPinterest').val() != '')
				data += '[rb_link type="pinterest" href="' + $('#rbSocialPinterest').val() + '" /]';
			if($('#rbSocialrb_linkedIn').val() != '')
				data += '[rb_link type="linkedin" href="' + $('#rbSocialLinkedIn').val() + '" /]';
			if($('#rbSocialVimeo').val() != '')
				data += '[rb_link type="vimeo" href="' + $('#rbSocialVimeo').val() + '" /]';
			if($('#rbSocialLastFM').val() != '')
				data += '[rb_link type="lastfm" href="' + $('#rbSocialLastFM').val() + '" /]';
			if($('#rbSocialTumblr').val() != '')
				data += '[rb_link type="tumblr" href="' + $('#rbSocialTumblr').val() + '" /]';
			if($('#rbSocialForrst').val() != '')
				data += '[rb_link type="forrst" href="' + $('#rbSocialForrst').val() + '" /]';
			if($('#rbSocialSkype').val() != '')
				data += '[rb_link type="skype" href="' + $('#rbSocialSkype').val() + '" /]';
			if($('#rbSocialFlickr').val() != '')
				data += '[rb_link type="flickr" href="' + $('#rbSocialFlickr').val() + '" /]';
			if($('#rbSocialDeviantart').val() != '')
				data += '[rb_link type="deviantart" href="' + $('#rbSocialDeviantart').val() + '" /]';
			if($('#rbSocialGoogle').val() != '')
				data += '[rb_link type="google" href="' + $('#rbSocialGoogle').val() + '" /]';
			if($('#rbSocialFriendFeed').val() != '')
				data += '[rb_link type="friendfeed" href="' + $('#rbSocialFriendFeed').val() + '" /]';
		...
		function insertTabs(){
			var data = '[rb_tabs style="' + $('#rbTabsStyle').val() + '"]'
		...
		function insertTagline(){
			var data = '[rb_tagline]' + $('#rbTaglineContent').val() + '[/rb_tagline]';
			return data;
		}

		function insertTitleTagline(){
			var data = '[rb_title_tagline title="' + $('#rbTitleTaglineTitle').val() + '"]' + $('#rbTitleTaglineTagline').val() + '[/rb_title_tagline]';
			return data;
		}
		...
		function insertTextBox(){
			var data = '[rb_text_box]' + $('#rbTextBoxContent').val() + '[/rb_text_box]';
			return data;
		}
		...
		function insertTestimonials(){
			...
			$('#rbTestimonials').children('div.boxes').children('div').each(function(){
				data += '[rb_testimonial source="' + $(this).find('.rbTestimonialsSource').val() + '"]' + $(this).find('.rbTestimonialsContent').val() + '[/rb_testimonial]';
			});
