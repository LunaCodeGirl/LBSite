	...
	function widget( $args, $instance ) {
		...
		echo '<div style="position:relative;"><div class="minigallery  listfade-img">';
			...
			foreach($imagelinks as $imagelink):
				...
				$columndiv = "one_half ";
					echo '<div class="covered tp_imgholder '.$columndiv.$lastelement.' hovering" style="overflow:hidden;" ><img class="" style="margin-bottom: -5px;width:93px;" src="'.$imagelink.'"><div class="hovering_more"><a href="'.$imagelink.'" rel="prettyPhoto['.$unique.']" title="'.$currenttitle.$currentdesc.'"><div class="pmore"></div></a></div></div>';
...
class averisPosts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		foreach ($poplist as $poppost) :  setup_postdata($poppost);
				...
				$blogimageurl = wp_get_attachment_url( get_post_thumbnail_id($poppost->ID) ); 
                if ($blogimageurl != "") {
                	echo '<div class=" listfade-img blogposts" '.$last.' style="margin-bottom:10px;">
...
class averisQuickcontact extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
	   	...
	   	if ( $title ) echo $before_title . $title . $after_title;
		echo '<div class="reply-form four columns alpha"><form id="quickcontact" method="post" action="#"><input type="hidden" name="sendto" id="sendto" value=\''.str_replace("@","§",$instance['sendto']).'\'/>
