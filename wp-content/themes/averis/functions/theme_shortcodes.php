...
function vid_youtube( $atts ) {
   ...
   return '<div class="bordered scalevid"><iframe src="http://www.youtube.com/embed/'.$video_id.'?hd=1&amp;wmode=opaque&amp;controls=1&amp;showinfo=0&amp;autohide=1" width="'.$width.'" height="'.$height.'"></iframe></div>';
}
...
function vid_vimeo( $atts ) {
   ...
   return '<div class="bordered scalevid"><iframe src="http://player.vimeo.com/video/'.$video_id.'?title=0&amp;byline=0&amp;portrait=0&amp;autohide=1&color='.str_replace("#", "", get_option_tree("aversis_main_color")).'" width="'.$width.'" height="'.$height.'"></iframe></div>';
}
	...
	function latest_posts_build( $atts ) {
		...
		foreach ($poplist as $poppost) :  
	            ...
	            if($type=="text"){
	                if ($blogimageurl != "") {
	                	$return_list .= '<li>
										<div class=" tp_teaser_imgholder"><a href="'.$base.'/'.$repl.'/'.$poppost->post_name.'"><img src="'.$blogimageurl.'"></a></div>
	...
	function latest_projects_build( $atts ) {
		...
		foreach ($poplist as $poppost) :  
	            ...
	            if($type=="text"){
	                if ($blogimageurl != "") {
	                	$return_list .= '<li>
										<div class="tp_teaser_imgholder hovering"><a href="'.$base.'/'.$repl.'/'.$poppost->post_name.'"><img src="'.$blogimageurl.'"></a><div class="hovering_link notalone"><a href="#"><div class="plink"></div></a></div><div class="hovering_more notalone"><a href="#"><div class="pmore"></div></a></div></div>
										<div class="teaser_topline"><a href="'.$base.'/'.$repl.'/'.$poppost->post_name.'">'.$poppost->post_title.'</a></div>';
										if($cat=="yes"){
											$tax_cats = "";
				...
				elseif ($type=="image") {
					if($blogimageurl != ""){
						$return_list .= '<li>
										<div class="tp_teaser_imgholder"><a href="'.$base.'/'.$repl.'/'.$poppost->post_name.'"><img src="'.$blogimageurl.'"></a><div class="hovering_link notalone"><a href="#"><div class="plink"></div></a></div><div class="hovering_more notalone"><a href="#"><div class="pmore"></div></a></div></div>
										<div class="portfolio">
											<div class="topline"><a href="'.$base.'/'.$repl.'/'.$poppost->post_name.'">'.$poppost->post_title.'</a></div>';
						if($cat=="yes"){
							$tax_cats = "";
							...
							$return_list .='<div class="subline">'.$averis_in.' '.substr($tax_cats, 2).'	</div>';
						}	
						$return_list .= '			</div>
								</li>';
...
// TEASER CUSTOM
	...
	function teaser_item_build($atts,$content = null){
		...
		$return_list = '<li><div class="tp_teaser_imgholder hovering"><img src="'.$image.'"><div class="hovering_link"><a href="'.$href.'"><div class="plink"></div></a></div><!--div class="hovering_more notalone"><a href="#"><div class="pmore"></div></a></div--></div><div class="teaser_topline teaser_without_subinfos"><a href="'.$href.'">'.$title.'</a></div><p>'.do_shortcode($content).'</p></li>';
...
// SLIDER
	function slider_build( $atts ) {
	    ...
	    if(is_array($banner_array)){
	    	...
	    	$banner_id = $banner_slug_array[$banner_counter];
	    	// Banner HTML
...
function add_sc_select(){
	...
	// VIDEOS
		...
		/* Youtube Video */
		$shortcodes_list .= "<option value='[video_youtube video_id=\"Your Video ID goes here\" width=\"460\" height=\"259\"]'>Youtube Video</option>";
		/* Vimeo Video */
	...
	// LIGHTBOX
		...
		/* prettyPhoto Vimeo */
		$shortcodes_list .= "<option value='<a href=\"http://vimeo.com/VIMEO VIDEO ID GOES HERE&width=720&height=405\" data-rel=\"prettyPhoto[folio]\" title=\"ENTRY DESCRIPTION TEXT GOES HERE\"><img class=\"scale-with-grid lightbox alignleft\" src=\"LINK TO THUMB IMAGE GOES HERE\" alt=\"\" /></a>'>Lightbox Vimeo Video with Gallery Style</option>";
	// Price Tables
...
// MISC
    ...
    // ACCORDION
    	$shortcodes_list .= "<option value='<ul class=\"contentsc accordion\">
