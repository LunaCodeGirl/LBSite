			...
			switch ($pagecustoms["averis_post_type"]) {
				...
				case 'video':
					...
					if($pagecustoms["averis_video_type"]=="youtube"){
						$scalevid = "scalevid";
						$post_top = '<iframe src="http://www.youtube.com/embed/'.$pagecustoms["averis_youtube_id"].'?hd=1&wmode=opaque&autohide=1&showinfo=0" width="'.$video_width.'" height="'.$video_height.'" style="margin-bottom: -6px;" frameborder="0"></iframe>';
					}
					elseif ($pagecustoms["averis_video_type"]=="vimeo") {
						$scalevid = "scalevid";
						$post_top = '<iframe src="http://player.vimeo.com/video/'.$pagecustoms["averis_vimeo_id"].'?title=0&byline=0&portrait=0&color='.str_replace("#", "", $highlight_color).'" width="'.$video_width.'" height="'.$video_height.'" style="margin-bottom: -6px;" frameborder="0"></iframe>';
					}
            			...
            			foreach ( $slider_slugs as $slug ){
                			$checked="";
                		 	if($slug==$averis_slider) {
                		 		$post_top = do_shortcode('[averis_slider name="'.get_option($slug."banner_slug").'"]');
                		 	}
                		} 	
				break;
				default:
											...
											<div class="sharings_wrap">
												...
												<?php if($averis_blog_overview_socials_google!="off"){ ?>										
													<div class="sharings shgoogleplus">
														<div class="g-plusone" href="<?php the_permalink(); ?>" data-size="medium" data-count="true"></div>
													</div> 
												<?php } ?>
												<?php if($averis_blog_overview_socials_pinterest!="off"){ ?>	
													<div class="sharings shpinterest">
														<a href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode(get_permalink()); ?>&media=<?php echo urlencode($blogimageurl); ?>&description=<?php echo excerpt(25); ?>" class="pin-it-button" count-layout="horizontal"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" /></a>
													</div>
												<?php } ?>
...
<?php if ($averis_detail_view_style != "full"){?>	
	<!-- Post Comments -->
	<div class="clear"></div>
	<div style="sixteen columns" style="clear:both"> 
