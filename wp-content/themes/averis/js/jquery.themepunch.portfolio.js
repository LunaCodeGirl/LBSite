			...
			function arrangeMore(main,opt) {
				...
				main.find('.pmore').each(function(i) {
					...
					button.click(function() {
						...
						li.closest('ul').find('li').each(function() {
							...
							mli.animate({'opacity':0.2},{duration:200,queue:false});
						...
						if (li.data('html') !=undefined)  {
							var spae=0;							
							if (li.data('html').split('bannercontainer').length>1) spae=500;							
							setTimeout(function() { pd.find('.portfolio_detail_imgholder').html(li.data('html'));},spae);
						}  else {
							if (li.data('imgsrc') !=undefined)
								pd.find('.portfolio_detail_imgholder img').attr('src',li.data('imgsrc'));
						}
						
						//alert(li.data('html'));	
						pd.find('.topline').html(li.data('title'));
						pd.find('.subline').html(li.data('detail_subline'));
						pd.find('#detail_innerhtml').html(li.data('detail_innerhtml'));
						
						pd.find('.subline span:last').remove();
						// WAIT FOR THE LOADING OF THE BIG IMAGE
						pd.waitForImages(
							function() {  
								setTimeout(function() {pd.fadeIn(300);jQuery(".scalevid").fitVids();},50);
								
								$('body,html').animate({ scrollTop: ($('#portfolio_details_mask').offset().top - 50)+"px" }, { duration: 500});
							});
						return false;
					});
				});
				
				bod.find('.portfolio_close').click(function() {
					pd.fadeOut(300);
					main.find('li').each(function() {
						...
						pd.find('.portfolio_detail_imgholder img').attr('src','');
					});
				});
				
				bod.find('.portfolio_right').click(function() {					
					searchNextLi(main.find('li.selected'));					
				});
				
				bod.find('.portfolio_left').click(function() {					
				...
				bod.find('#portfolio_details_mask').each(function() {
					...
					setInterval(function() {
						...
						mask.animate({'height':newh},{duration:200,queue:false});
					},100);
				});
