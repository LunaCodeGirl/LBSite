...
*/	
	if ( empty($contact_form_fields) || !is_array($contact_form_fields) ) {
		...
		$default_form = '
		[contact-field label="'.__('Name','framework_localize').'" type="name" required="true" /]
		[contact-field label="'.__('Email Address','framework_localize').'" type="email" required="true" /]
		[contact-field label="'.__('Website','framework_localize').'" type="url" /]';
		if ( 'yes' == strtolower($grunion_form->show_subject) )
			$default_form .= '
			[contact-field label="'.__('Subject','framework_localize').'" type="subject" /]';
		$default_form .= '
		[contact-field label="'.__('Comments','framework_localize').'" type="textarea" /]';
...
*/
	...
	if ( $field['type'] == 'email' ) {
		...
		$r .= "\t<div>\n";
		$r .= "\t\t<input type='checkbox' name='".esc_attr($field_id)."' value='".__('Yes','framework_localize')."' class='".esc_attr($field['type'])."' ".( $field_value ? "checked='checked' " : "")." />\n";
...
function contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && $field['type'] == 'email' ) {
		if ( !is_email( $field_value ) ) {
			...
			$contact_form_errors->add( $field_id, sprintf( __( '%s requires a valid email address','framework_localize' ), $field['label'] ) );
		}
		...
		$contact_form_errors->add( $field_id, sprintf( __('%s is required','framework_localize'), $field['label'] ) );
	}
...
*/
	...
	$errors = array();
	if ( is_wp_error( $contact_form_errors ) && $errors = (array) $contact_form_errors->get_error_codes() ) {
		$r .= "<div class=\"notification error closeable\"><p style=\"font-size:12px;\">\n";
		foreach ( $contact_form_errors->get_error_messages() as $message )
			$r .= "\t- $message<br />";
		$r .= "</p>";
		$r .= "\n</div>\n\n";
...
*/
	...
	$resp = tt_recaptcha_check_answer ($captcha_private,$_SERVER["REMOTE_ADDR"],$_POST["recaptcha_challenge_field"],$_POST["recaptcha_response_field"]); //verify captcha entered with server.
 	    ...
 	    if (!$resp->is_valid) {
			...
			$new_contact_form_errors->add("$id-recaptcha", sprintf( __('Invalid CAPTCHA','framework_localize'), '' ) );
...
*/
	...
	$time = date_i18n( __('l F j, Y \a\t g:i a','framework_localize'), current_time( 'timestamp' ) );
...
function contact_form_init() {
	...
	register_post_type( 'feedback', array(
		'labels'	=> array(
			'name'			=> __( 'Messages','framework_localize' ),
			'singular_name'	=> __( 'Message','framework_localize' ),
			'search_items'	=> __( 'Search Message','framework_localize' ),
			'not_found'		=> __( 'No message found','framework_localize' ),
			'not_found_in_trash'	=> __( 'No message found','framework_localize' )
		),
...
function grunion_media_button( ) {
	...
	$title = esc_attr( __( 'Add a custom form','framework_localize' ) );
