	...
	function formate_shortcode()
	{
		...
		if( isset( $tt_shortcodes ) && is_array( $tt_shortcodes ) )
		{
			...
			foreach( $this->params as $pkey => $param )
			{
				...
				switch( $param['type'] )
				{
					case 'text' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<input type="text" class="tt-form-text tt-input" name="' . $pkey . '" id="' . $pkey . '" value="' . $param['std'] . '" />' . "\n";
						$output .= $row_end;
					...
					case 'textarea' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<textarea rows="10" cols="30" name="' . $pkey . '" id="' . $pkey . '" class="tt-form-textarea tt-input">' . $param['std'] . '</textarea>' . "\n";
						$output .= $row_end;
					...
					case 'select' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<select name="' . $pkey . '" id="' . $pkey . '" class="tt-form-select tt-input">' . "\n";
					...
					case 'checkbox' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<label for="' . $pkey . '" class="tt-form-checkbox">' . "\n";
						$output .= '<input type="checkbox" class="tt-input" name="' . $pkey . '" id="' . $pkey . '" ' . ( $param['std'] ? 'checked' : '' ) . ' />' . "\n";
			...
			if( isset( $tt_shortcodes[$this->popup]['child_shortcode'] ) )
			{
				...
				$prow_start .= '<td><a href="#" id="form-child-add" class="button-secondary">' . $tt_shortcodes[$this->popup]['child_shortcode']['clone_button'] . '</a>' . "\n";
				...
				foreach( $this->cparams as $cpkey => $cparam )
				{
					...
					switch( $cparam['type'] )
					{
						case 'text' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<input type="text" class="tt-form-text tt-cinput" name="' . $cpkey . '" id="' . $cpkey . '" value="' . $cparam['std'] . '" />' . "\n";
							$coutput .= $crow_end;
						...
						case 'textarea' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<textarea rows="10" cols="30" name="' . $cpkey . '" id="' . $cpkey . '" class="tt-form-textarea tt-cinput">' . $cparam['std'] . '</textarea>' . "\n";
							$coutput .= $crow_end;
						...
						case 'select' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<select name="' . $cpkey . '" id="' . $cpkey . '" class="tt-form-select tt-cinput">' . "\n";
						...
						case 'checkbox' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<label for="' . $cpkey . '" class="tt-form-checkbox">' . "\n";
							$coutput .= '<input type="checkbox" class="tt-cinput" name="' . $cpkey . '" id="' . $cpkey . '" ' . ( $cparam['std'] ? 'checked' : '' ) . ' />' . "\n";
